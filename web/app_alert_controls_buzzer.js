(function () {
  ...
  async function refreshAlertStatus() {
    try {
      ...
      const output = data.player || data.output;
      if (!data.enabled || !output) {
        hint.textContent = '自动告警未启用或未检测到蜂鸣器/扬声器';
      ...
      setState(output === 'rosmaster_buzzer' ? '蜂鸣器就绪' : '扬声器就绪', 'is-ok');
    } catch (_) {
